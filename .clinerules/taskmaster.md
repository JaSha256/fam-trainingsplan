...
covering both the recommended MCP tools, suitable for integrations like Cline,
...
- **Usage:** Run this once at the beginning of a new project, typically via an
  integrated tool like Cline. Operates on the current working directory of the
  MCP server.
...
- **CRITICAL INFORMATION** If you need to collect information from multiple
  ...
  not needlessly get tasks one at a time if you need to get many as that is
  wasteful.
...
within the `env` section of your **`.cline/mcp.json`** file (for MCP/Cline
...
[dev_workflow.md](.clinerules/dev_workflow.md).
