...
1.  **MCP Server (Recommended for Integrated Tools)**:
    - For AI agents and integrated development environments (like Windsurf),
...
`.windsurf/mcp.json`. **If AI commands FAIL in CLI** verify that the API key for
...
- **Available Profiles**: Claude Code, Cline, Codex, Windsurf, Roo Code, Trae,
  Windsurf (claude, cline, codex, windsurf, roo, trae, windsurf)
- **During Initialization**: Use `task-master init --rules windsurf,windsurf` to
...
- **Rule Structure**: Each profile creates its own directory (e.g.,
  `.windsurf/rules`, `.roo/rules`) with appropriate configuration files
